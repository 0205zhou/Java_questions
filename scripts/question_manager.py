...
def slugify(text: str, max_len: int = 60) -> str:
    """生成文件名（保留中文标点，仅去除 Windows 非法字符）"""
    # 文件名 = 问题名（用户约定：文件名与问题名一致）
    # 仅去除 Windows 非法字符 \ / : * ? " < > | 和前后空白
    ...
    def add(self, question: str, answer: str, *,
        ...
        if cat not in self.index["categories"]:
            ...
            self.index["categories"][cat] = {
                "name": cat,
                "icon": "📁",
                "description": CATEGORY_KEYWORDS.get(cat, []),
                "count": 0,
            }
            (DATABASE_DIR / cat).mkdir(parents=True, exist_ok=True)
        ...
        # 如果答案内容以同名标题开头，去掉重复标题（add 时会重写 # {question}）
        if answer.strip().startswith(f"# {question}"):
