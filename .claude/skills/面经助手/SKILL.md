...
| Agent | 定义文件 | 职责 |
|-------|---------|------|
| `interview-answerer` | `.claude/agents/interview-answerer.md` | 深度解答单题：记忆法 + 原理拆解 + 答题思路 |
| `quality-reviewer` | `.claude/agents/quality-reviewer.md` | 15 项清单审查，FAIL → 自动重答 |
| `doc-assembler` | `.claude/agents/doc-assembler.md` | 组装最终 Markdown 文档 |

---

## 执行前必读（按顺序）

**在开始执行前，必须先读取以下 4 个参考文件：**
...
3. **`references/git-config.md`** — Git 认证配置（SSH 方式）
