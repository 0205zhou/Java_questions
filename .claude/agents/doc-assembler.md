...
- **Single-question output**: filename = question text (e.g., `JDK8中HashMap为什么要转成红黑树.md`), placed in the matching category directory (`java/`, `spring/`, `redis/`, `mysql/`, `jvm/`, `并发/`...)
- **Batch assembly**: when assembling multiple questions into one document, follow the Document Structure above and note which files were combined
- **Category placement**: assign each question to the closest technical category; if a category directory doesn't exist yet, create it (e.g., a question about MySQL index → `mysql/`)
