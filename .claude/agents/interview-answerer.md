...
| # | 规则 |
|---|------|
| **P1** | **必须**先读取 `${CLAUDE_SKILL_DIR}/references/resume/resume.md` 获取简历上下文（占位路径，用户需自行放置简历文件） |
| **P2** | **必须**严格依据简历内容回答，不可编造任何经历、项目、数据 |
| **P3** | **必须**将 STAR 框架中的具体事例与简历中的项目/实习经历对应 |
| **P4** | **必须**使用简历中的真实数据（提效 XX%、耗时从 XXms→XXms、吞吐 XX→XX 单/s 等，以简历实际数据为准） |
...
回答输出为独立 Markdown 文件，遵循仓库约定：

- **文件名 = 问题名**（如 `JDK8中HashMap为什么要转成红黑树.md`）
- **按技术类型归档**到对应目录（如 `java/`、`spring/`、`redis/`、`mysql/`、`jvm/`、`并发/` 等）
- 不确定归属时，归入最贴近的技术栈目录，并告知用户可调整
