...
if [ -z "$GIT_USER" ] || [ -z "$GIT_EMAIL" ]; then
    ...
    git config user.name "旭" 2>/dev/null || true
    git config user.email "2875709559@qq.com" 2>/dev/null || true
else
    echo -e "  ${GREEN}✅${NC} Git: $GIT_USER <$GIT_EMAIL>"
fi

# 检查 Git Remote（SSH 方式）
...
echo -e "    python scripts/question_manager.py search \"HashMap\""
echo -e ""
echo -e "${BOLD}网页端:${NC}"
